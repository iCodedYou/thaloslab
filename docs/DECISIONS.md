...
| `DEFERRED-PENDING-MULTI-MACHINE` (collab — engine→dispatch: **BUILT G0+G1, live G2 pending**) | the daemon's engine DISPATCHING a task to an admitted, routable collab peer. **BUILT + gate-green (2026-07-01):** G0 (`collab-route.ts` `resolveCollabRoute`) — a fail-closed gate: dispatch ONLY when the project opted in (`routingPolicy.collab`) AND the task names a routable `collab:<peer>:<vendor>` AND differ-by-vendor holds; else PARK; `'auto'` NEVER collab (mutation-proven: bypass the gates → 3 park tests red). G1 — the StageRunner builds `makeCollabAdapter` INLINE over the live PeerLink and dispatches via `executeRun` wrapped in try/catch → fail-closed on `PeerRevokedError`/`SecretLeakError`/`PeerInvokeTimeoutError` (mutation-proven: re-throw → the revoke/secret/timeout tests raw-throw out of `run()` instead of escalating; a revoked peer's patch never reaches quarantine). In-process `--mock` round-trip through the REAL `StageRunner.run` proves pack→push→quarantine→host-git re-derive→re-gate + manifest + lying-peer-caught; Wire D unchanged. **What remains = G2 ONLY:** the LIVE `--mock` round-trip through the real daemon to the real Mac peer | schedule G2 (cofounder + tailnet + firewall 8474): a `routingPolicy.collab=true` project + a ticket whose agent targets `collab:mac-1:<vendor>`, one `--mock` round-trip through the daemon |
